...
- (NSString *)adWhirlApplicationKey {
    return @"2f374d75c2bf4c7caf852cfb134bbdb7";
}
...
- (void)adWhirlDidReceiveAd:(AdWhirlView *)adWhirlVieww {
    CGSize adSize = [adWhirlVieww actualAdSize];
    CGRect newFrame = adWhirlVieww.frame;
    ...
    adWhirlVieww.frame = newFrame;
    // ... adjust surrounding views here ...
}

#pragma mark Helpers
// Check for clonflic state
...
// iCloud isn't switched on, but it's available
// iCloud isn't available, but it was on before
// iCloud was switched on (but wasn't previously)
// iCloud was switched off (but wasn't previously)
-(BOOL)iCloudOn {
...
-(NSString*)getDocFilename:(NSString*)prefix uniqueInObjects:(BOOL)uniqueInObjects {
    ...
    // At this point, the documentation list should be up-to-date
    BOOL done = NO;
    BOOL first = YES;
    while (!done) {
        if (first) {
...
-(BOOL)renameEntry:(PTKEntry *)entry to:(NSString*)filename {
    ...
    if ([self docNameExistsInObjects:newDocFilename]) {
        NSString *message = [NSString stringWithFormat:@"\"%@\" si already taken, Please choose a different name", filename];
...
- (void)iCloudToLocal {
    ...
    UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:@"You're Not Using iCloud" 
                                                            message:@"What would you like to do with the documents currently on this iPad?" 
                                                            delegate:self 
                                                            cancelButtonTitle:@"Continue Using iCloud" 
                                                            otherButtonTitles:@"Keep a Local Copy", @"Keep on iCloud Only", nil];
    alertView.tag = 2;
    [alertView show];
...
-(void)processiCloudFiles:(NSNotification*)notification {
    ...
    // The query reports all files found, everytime
    ...
    if ([self iCloudOn]) {
        ...
        // Iterate backwwards because we need to remove items from the array
        ...
        if (!_iCloudAvailable) {
            ...
            if ([self iCloudWasOn]) {
                UIAlertView * alertView = [[UIAlertView alloc] 
                                           initWithTitle:@"You're Not Using iCloud" 
            ...
            // Ask user if want to turn iCloud if it's available adn we haven't asked already
            ...
            // Strat querying iCloud for files, whether on or off
            [self startQuery];
...
- (void)viewDidLoad
{
    ...
    // Get device type from singleton
    MyManager *sharedManager = [MyManager sharedManager];
    deviceType = sharedManager.deviceType;
    ...
    // In case user goes to Settings and toggle iCloud settings
